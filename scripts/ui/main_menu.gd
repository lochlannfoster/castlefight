...
func _ready():
    ...
    var _mp_connect_result = $VBoxContainer/MultiplayerButton.connect("pressed", Callable(self, "_on_multiplayer_button_pressed"))
    if _mp_connect_result != OK:
        push_warning("Failed to connect multiplayer button signal")
    
    var _options_connect_result = $VBoxContainer/OptionsButton.connect("pressed", Callable(self, "_on_options_button_pressed"))
    if _options_connect_result != OK:
        push_warning("Failed to connect options button signal")
    
    var _quit_connect_result = $VBoxContainer/QuitButton.connect("pressed", Callable(self, "_on_quit_button_pressed"))
...
func _on_multiplayer_button_pressed():
    ...
    if game_manager and game_manager.has_method("change_scene_to_file"):
        var _scene_change_result = game_manager.change_scene_to_file("res://scenes/lobby/lobby.tscn")
    else:
        # Fallback if not available
        var _scene_change_result = get_tree().change_scene_to_file("res://scenes/lobby/lobby.tscn")
...
func verify_critical_assets():
    ...
    # Check shaders
    var shader_path = "res://shaders/fog_of_war.gdshader"    if FileAccess.file_exists(shader_path):
