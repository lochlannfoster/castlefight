...
func _create_building_menu() -> void:
	...
	building_menu.set_anchors_preset(Control.PRESET_BOTTOM_LEFT)
	building_menu.margin_left = 10
	building_menu.margin_top = -110
	building_menu.margin_right = 410
	building_menu.margin_bottom = -10
...
# Signal handlers
func _on_resources_changed(team: int, resource_type: int, amount: float) -> void:
	if team == current_team:
		update_resource_display()

func _on_income_changed(team: int, amount: float) -> void:
	if team == current_team:
		update_income_display()
