...
func _ready() -> void:
    ...
    if not has_node("ModeTabContainer/Create Game/CreateGamePanel/DebugInfoLabel"):
        ...
        debug_info_label.position = Vector2(20, 410)
        debug_info_label.size = Vector2(480, 40)
        debug_info_label.horizontal_alignment = Label.ALIGNMENT_CENTER
        debug_info_label.add_theme_color_override("font_color", Color(1, 0.5, 0, 1)) # Orange color
    ...
    debug_mode_checkbox.connect("toggled", Callable(self, "_on_debug_mode_toggled"))
    ...
    create_team_a_button.connect("pressed", Callable(self, "_on_team_a_button_pressed"))
    create_team_b_button.connect("pressed", Callable(self, "_on_team_b_button_pressed"))
    
    connect_button.connect("pressed", Callable(self, "_on_connect_button_pressed"))
    join_team_a_button.connect("pressed", Callable(self, "_on_team_a_button_pressed"))
    join_team_b_button.connect("pressed", Callable(self, "_on_team_b_button_pressed"))
    ...
    join_selected_button.connect("pressed", Callable(self, "_on_join_selected_button_pressed"))
    
    back_button.connect("pressed", Callable(self, "_on_back_button_pressed"))
    
    # Connect dialog signals
    connection_dialog.get_node("VBoxContainer/CancelButton").connect("pressed", Callable(self, "_on_connection_cancel_pressed"))
    
    # Connect NetworkManager signals
    network_manager.connect("server_started", Callable(self, "_on_server_started"))
    network_manager.connect("server_stopped", Callable(self, "_on_server_stopped"))
    ...
    network_manager.connect("player_list_changed", Callable(self, "_on_player_list_changed"))
    network_manager.connect("match_ready", Callable(self, "_on_match_ready"))
    network_manager.connect("network_error", Callable(self, "_on_network_error"))
...
func create_game() -> void:
    ...
    # Validate input
    if game_name.is_empty() or player_name.is_empty():
...
func connect_to_game() -> void:
    ...
    # Validate input
    if player_name.is_empty() or ip.is_empty():
