...
func _create_required_scenes() -> void:
	...
	if FileAccess.file_exists(scene_creator_path):
...
func _create_basic_race_selection() -> void:
	...
	human_button.connect("pressed", Callable(self, "_on_race_selected").bind("human", 0))
	vbox.add_child(human_button)
	
	var orc_button = Button.new()
	orc_button.text = "Orc Horde"
	orc_button.connect("pressed", Callable(self, "_on_race_selected").bind("orc", 0))
	vbox.add_child(orc_button)
	
	var start_button = Button.new()
	start_button.text = "Start Game"
	start_button.connect("pressed", Callable(self, "_on_race_selection_complete"))
...
func validate_and_restore_game_state() -> bool:
	...
	if not grid_system or grid_system.grid_cells.is_empty():
...
func _create_default_file(file_path: String) -> void:
	# Check if file already exists	if FileAccess.file_exists(file_path):
...
func ensure_icon_exists() -> void:	if not FileAccess.file_exists("res://icon.png"):
...
func change_scene_to_file(scene_path: String, _transition: bool = false) -> bool:
	...
	# Verify scene exists before trying to load it	if !FileAccess.file_exists(scene_path):
		...
		if default_scene_path != "" and FileAccess.file_exists(default_scene_path):
...
func does_scene_exist(scene_path: String) -> bool:	return FileAccess.file_exists(scene_path)
