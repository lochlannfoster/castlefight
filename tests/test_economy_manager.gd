...
func test_add_resource():
	...
	# Try negative amount (spend resources)
	economy_manager.add_resource(team, economy_manager.ResourceType.GOLD, -20)
	new_gold = economy_manager.get_resource(team, economy_manager.ResourceType.GOLD)
	assert_eq(new_gold, initial_gold + amount - 20)

func test_income():
	var team = 0
	var initial_income = economy_manager.get_income(team)
	
	# Add income
	var amount = 5
	economy_manager.add_income(team, amount)
	
	# Check new value
	var new_income = economy_manager.get_income(team)
	assert_eq(new_income, initial_income + amount)
	
	# Test distribute income
	var initial_gold = economy_manager.get_resource(team, economy_manager.ResourceType.GOLD)
	economy_manager._distribute_income()
	var new_gold = economy_manager.get_resource(team, economy_manager.ResourceType.GOLD)
	assert_eq(new_gold, initial_gold + new_income)
...
func test_can_afford_building():
	...
	# Reduce gold
	economy_manager.team_resources[team][economy_manager.ResourceType.GOLD] = 99
	
	# Now can't afford barracks
	assert_false(economy_manager.can_afford_building(team, "barracks"))
